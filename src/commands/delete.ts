module.exports = {
  name: "delete",
  description: "Delete a opened ticket",
  run: (client, message, args, db) => {
    ...
    if (hasStaff === false)
      return message.channel.send(
        "No staff roles were found.. No one can use this command"
      );
    staffRoles.some((r) => {
      if (message.member.roles.cache.has(r)) isStaff = true;
    });

    if (!isStaff) return message.channel.send("You are not a staff member");

    message.channel.delete().then(() => {
      db.splice(
        `tickets_${message.guild.id}_ticketChannels`,
        message.channel.id
      );
